...
public class ServiceFW extends VpnService implements Runnable {
    ...
    private Builder buildVPN(String netcontrolCommand)
    {
        ...
        while (it.hasNext())
        {
            ...
            if ((app.internet) && (app.appInfoExtra != null)) {
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP)
                {
                    ...
                    if (app.fw >= 30) {
                        // App is BLOCKED by package name, but in reality
                        // it is by UID
    ...
    public static void boot(Context context) {
        ...
        serviceIntent.putExtra("command", Global.NETCONTROL_BOOT);
    ...
    public static void replace(Context context) {
        ...
        if (Global.getNetControlState() == true) {
            Intent serviceIntent = new Intent(context, ServiceFW.class);
            serviceIntent.putExtra("command", Global.NETCONTROL_REPLACE);

            ContextCompat.startForegroundService(context, serviceIntent);
        }
    }

    private void startVPN(String netcontrolCommand) {

        Logs.myLog("ServiceFW - startVPN()", 2);

        Builder builder = buildVPN(netcontrolCommand);
        ...
        try {
            ...
            errorText = "ServiceFW - NetControl Security Exception:" + ex;
            Logs.myLog(errorText + ": " + ex, 1);
            // Logs.myLog("This is a KNOWN Android bug :-( ", 2);
            vpnInterface = null;
        } catch (IllegalStateException ex2) {
            errorText = "ServiceFW - NetControl State Exception: " + ex2;
            Logs.myLog(errorText + ": " + ex2, 1);
            vpnInterface = null;
        } catch (IllegalArgumentException ex3) {
            errorText = "ServiceFW - NetControl Argument Exception: " + ex3;
            Logs.myLog(errorText + ": " + ex3, 1);
            vpnInterface = null;
        } catch (Exception ex4) {
            errorText = "ServiceFW - NetControl Argument Exception: " + ex4;
        ...
        if (vpnInterface == null) {
            ...
            // Logs.myLog("Cannot Start NetControl!" , 0);
            if (errorText.equals("")) { // if boot...
                if (netcontrolCommand.equals(Global.NETCONTROL_BOOT)) {
                    notifyNetControlState(Global.getContext().getString(R.string.notify_netcontrol_fail3));
                } else {
                    notifyNetControlState(Global.getContext().getString(R.string.notify_netcontrol_fail1));
                }
            } else {
                notifyNetControlState(String.format(Global.getContext().getString(R.string.notify_netcontrol_fail2),errorText));
            }
        } else {

            Logs.myLog("ServiceFW - Created VPN interface", 3);

            if (netcontrolCommand.equals(Global.NETCONTROL_START)) {
                Logs.myLog("NetControl Enabled!", 1);
            }

            if (netcontrolCommand.equals(Global.NETCONTROL_BOOT)) {
                Logs.myLog("NetControl Enabled On Boot!", 1);
            }

            if (netcontrolCommand.equals(Global.NETCONTROL_WIDGET)) {
                Logs.myLog("NetControl Enabled By Widget!", 1);
            }

            if (netcontrolCommand.equals(Global.NETCONTROL_QS)) {
                Logs.myLog("NetControl Enabled By Quick Panel!", 1);
            }

            if (netcontrolCommand.equals(Global.NETCONTROL_REPLACE)) {
                Logs.myLog("NetControl Enabled By Upgrade!", 1);
            }

            if (netcontrolCommand.equals(Global.NETCONTROL_RESTART)) {
                Logs.myLog("NetControl Restarted due to config change!", 2);
            }
            ...
            Global.setNetControlState(true);
        }
        sendAppBroadcast(Global.NETCONTROL_STATE_CHANGE);
    ...
    private void stopVPN()
    {
        ...
        Global.setNetControlState(false);
        if (vpnInterface != null)
        {
            try {
                vpnInterface.close();
        ...
        if (notificationManager != null) {
            ...
            // But can replace
            sendNotificationServiceStop("NetControl Stopped");
        }
        sendAppBroadcast(Global.NETCONTROL_STATE_CHANGE);
    ...
    public void onRevoke() {
        ...
        if (Global.getNetControlState() == true)
        {
            notifyNetControlState("NetControl revoked by Android!");

            Global.setNetControlState(false);
            // Logs.myLog("NetControl Terminated!" , 0);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            ...
            if (command != null) {
                ...
                //
                if (command.equals(Global.NETCONTROL_START)) { // Only possible via GUI
                    Logs.myLog("ServiceFW - received start intent", 2);
                    startVPN(command);
                }
                if (command.equals(Global.NETCONTROL_RESTART)) {
                    Logs.myLog("ServiceFW -  received restart intent", 2);
                    startVPN(command);
                }

                if (command.equals(Global.NETCONTROL_STOP)) { // close an old VPN
                    Logs.myLog("ServiceFW - received stop intent", 2);
                    stopVPN();
                    Logs.myLog("ServiceFW - NetControl Disabled!" , 1);
                }
                if (command.equals(Global.NETCONTROL_STATUS)) {
                    Logs.myLog("ServiceFW - received status intent. Status: " + Global.getNetControlState(), 2);
                    if (vpnInterface == null) {
                        Global.setNetControlState(false);
                    } else {
                        Global.setNetControlState(true);
    ...
    public void onDestroy() {
        ...
        if (Global.getNetControlState() == true) {
            notifyNetControlState("NetControl destroyed by Android!");
        }
        Global.setNetControlState(false);
        ...
        while (!(Thread.currentThread().isInterrupted())) {
            try {
                ...
                if (Global.getAppList() == false)
                {
                    ...
                    // startVPN(Global.NETCONTROL_RESTART);
                }
            } catch (InterruptedException e) {
    ...
    private void notifyNetControlState(String message)
    {

        Logs.myLog("ServiceFW - notifyNetControlState: " + message, 2);
